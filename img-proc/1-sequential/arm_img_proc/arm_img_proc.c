...
int main(int argc, char ** argv)
{
  ...
  for (uint32_t f=1; f <= nr_files; f++) {
    ...
    if (bytes_snd > MAX_SIZE)
    {
      printf("Image of size %ux%ux%u does not fit in the shared mem %u %u\n", xsize_snd, ysize_snd, bitsperpixel_snd, bytes_snd, MAX_SIZE);
      break;
    }

    // send image to RISC-V
    data_in_shared_mem->xsize = xsize_snd;
    data_in_shared_mem->ysize = ysize_snd;
    data_in_shared_mem->bitsperpixel = bitsperpixel_snd;

    for (uint32_t i = 0; i < bytes_snd; ++i)
      data_in_shared_mem->frame[i] = frame_snd[i];

    data_in_shared_mem->sync = CONSUMER_IS_OWNER;

    // wait for the RISC-V to produce the output data
    while(data_in_shared_mem->sync != PRODUCER_IS_OWNER);
    ...
    if (writeBMP(outfile, (uint8_t*)data_in_shared_mem->frame, xsize_snd, ysize_snd, 8)) {
