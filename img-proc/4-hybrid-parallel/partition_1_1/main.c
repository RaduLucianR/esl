...
#define OUTPUT (&(MEM->fifo_out1))

int main()
{
  xil_printf("Overlay top running\n");
  
  if (myfifo_init(OUTPUT, MEM->buffer_out1, BUFFER_SIZE, sizeof(token_t)) == NULL)
    return -1;

  xil_printf("Overlay top output initialized: %u\n", OUTPUT->m_size);
  ...
  while (1)
  {
    ...
    xil_printf("Overlay top done\n");
    if (ysize != 0) MEM->sync++;
    else while(MEM->sync == 1);
  }
