...
#define OUTPUT (&vep_tile0_partition1_shared_region->fifo)
#define BUFFER (vep_tile0_partition1_shared_region->buffer)
#define INPUT (&MEM->fifo_in1)

int main()
{
  xil_printf("Greyscale top running\n");
  ...
  xil_printf("Greyscale top output initialized\n");
  if (myfifo_init(INPUT, MEM->buffer_in1, BUFFER_SIZE_RGB, sizeof(token_24_t)) == NULL)
  {
    xil_printf("Failed to initialize input fifo 1\n");
    return -1;
  }

  xil_printf("Greyscale top input initialized: %u\n", INPUT->m_size);
