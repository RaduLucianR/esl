...
{
  volatile void* to_write = myfifo_claim_space(fifo_admin);

  // If we cannot use memcpy, we have to copy the data by hand
  for (uint32_t i = 0; i < fifo_admin->m_token_size; ++i)
   *(BYTE*)(to_write + i) = *(BYTE*)(token_to_write + i);

  // memcpy(to_write, token_to_write, fifo_admin->m_token_size);
...
{
  volatile void* to_read = myfifo_claim_token(fifo_admin);

  for (uint32_t i = 0; i < fifo_admin->m_token_size; ++i)
   *(BYTE*)(new_token + i) = *(BYTE*)(to_read + i);

  // memcpy(new_token, to_read, fifo_admin->m_token_size);
